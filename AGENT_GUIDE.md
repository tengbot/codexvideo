...
`research -> proposal -> script -> scene_plan -> assets -> edit -> editable_timeline -> compose`
...
├── editable/           # Portable multi-track project, media, adapter drafts, and command history
├── assets/
...
Every production that reaches the edit stage must also create
`artifacts/editable_timeline.json` and a portable `editable/` project by calling
the `editable_timeline` tool with `operation="export"`. This is a supplementary
artifact: `edit_decisions` remains the renderer contract, while
`editable_timeline` is the human/agent round-trip contract.

The editable project must preserve stable element IDs, track placement, trims,
audio levels, text, basic keyframes, source provenance, regeneration metadata,
and an append-only command history. Manual or agent changes are applied through
the tool's `apply` operation and can be reverted with `undo`; use `import` to
translate the edited project back into schema-valid `edit_decisions` before the
next render.

HyperFrames and Remotion compositions remain source projects. For an external
NLE, render complex HTML/GSAP or React scenes as scene-level proxy clips and put
those proxies on the neutral timeline; do not replace the original runtime.
At publish, pass `editable_project_dir` to `export_bundle` so the deliverable
contains both the playable video and its editable project.

### Modular Campaign Contract

Use `campaign_batch` only after the agent and user have approved the creative
modules and experiment matrix. The agent remains the creative control plane;
the tool is a deterministic build executor for already-locked hook, body, CTA,
and delivery-profile choices.

A campaign lives in one normal project workspace and adds:
...
│   ├── variants.json
│   └── batch_run.json
├── outputs/<profile-id>/<variant-id>.mp4
└── reports/batch-report.json
```

Run `campaign_batch` with `operation="plan"` before rendering. The plan step
validates module roles, source paths, profile dimensions, experiment deltas,
and profile-specific reflow requirements, then creates stable jobs. Use
`operation="run"` or `operation="resume"` for mechanical execution.

Do not use FFmpeg center-crop as a substitute for responsive product or motion
graphics design. Profiles with `fit="reflow"` require every module to provide a
profile-specific source rendered by its approved HyperFrames or Remotion
composition. `contain` and `cover` are delivery treatments for footage or
explicitly approved fallback outputs.

Generated narration should use known TTS/script timing as subtitle truth.
Whisper transcription is the source for recorded speech and a post-render
comparison check, not a lossy replacement for timing the system already knows.

`campaign_batch` performs one final FFmpeg encode per deliverable, media QA,
content-hash caching, atomic state writes, and job-level resume. It must never
write hooks, choose experiment winners, generate unapproved combinations, or
publish performance claims. See `docs/CAMPAIGN_BATCH.md`.
