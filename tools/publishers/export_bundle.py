...
class ExportBundle(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "editable_project_dir": {
                "type": "string",
                "description": "Portable editable project directory to include with the final video.",
            },
            "platform": {
                "type": "string",
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        editable_project = inputs.get("editable_project_dir")
        if editable_project and not Path(editable_project).expanduser().is_dir():
            return ToolResult(
                success=False,
                error=f"editable_project_dir provided but not found: {editable_project}",
            )
        ...
        # Replace the transport copy on repeat exports so stale proxy media
        # cannot survive after an edit project changes.
        editable_project_path = None
        if editable_project:
            source_project = Path(editable_project).expanduser()
            editable_project_path = export_root / "editable-project"
            if editable_project_path.exists():
                shutil.rmtree(editable_project_path)
            shutil.copytree(source_project, editable_project_path)
            files_written.extend(
                str(path) for path in sorted(editable_project_path.rglob("*")) if path.is_file()
            )
        ...
        return ToolResult(
            ...
            data={
                ...
                **(
                    {"editable_project_path": str(editable_project_path)}
                    if editable_project_path else {}
                ),
            },
