...
class SubtitleGen(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "phrase_corrections": {
                "type": "object",
                "description": (
                    "Dictionary of multi-word ASR corrections applied before cue "
                    "grouping. Matching ignores surrounding punctuation and leading "
                    "hyphens. Example: "
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        phrase_corrections = inputs.get("phrase_corrections")
        ...
        if phrase_corrections:
            segments = self._apply_phrase_corrections(segments, phrase_corrections)
    ...
    def _apply_phrase_corrections(
        ...
        """Replace token sequences while preserving the matched time span."""
        import copy
        ...
        patterns = sorted(
            (
                (
                    [cls._normalized_token(token) for token in wrong.split()],
                    replacement,
                )
        ...
        while index < len(words):
            ...
            for pattern, replacement in patterns:
                count = len(pattern)
                candidate = [
                    cls._normalized_token(word.get("word", ""))
                    for word in words[index : index + count]
                ]
                if len(candidate) != count or candidate != pattern:
                    continue
                replacement_words = replacement.split()
                start = float(words[index]["start"])
                end = float(words[index + count - 1]["end"])
                step = (end - start) / max(len(replacement_words), 1)
                for replacement_index, replacement_word in enumerate(replacement_words):
                    corrected.append(
                        {
                            "word": replacement_word,
                            "start": round(start + replacement_index * step, 3),
                            "end": round(start + (replacement_index + 1) * step, 3),
                        }
                    )
                index += count
                matched = True
        ...
        return [
            {
                "id": 0,
                "start": corrected[0]["start"],
                "end": corrected[-1]["end"],
                "text": " ".join(word["word"].strip() for word in corrected),
                "words": corrected,
            }
        ]
    ...
    def _normalized_token(value: str) -> str:
        return re.sub(r"^[^\w]+|[^\w]+$", "", value.strip().lower())
