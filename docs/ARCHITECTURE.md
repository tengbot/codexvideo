...
7. **Campaign batches are frozen build graphs, not a second orchestrator** —
   the agent approves modules, variants, and delivery profiles in a
   `campaign_plan` artifact. `campaign_batch` only materializes and executes
   deterministic jobs, persists `batch_run`, and reports media QA. It cannot
   choose creative inputs or experiment winners.
