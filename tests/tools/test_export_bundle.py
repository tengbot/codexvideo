...
def test_export_includes_editable_project(tmp_path):
    video = tmp_path / "projects" / "demo" / "renders" / "final.mp4"
    _make_video(video)
    editable = tmp_path / "projects" / "demo" / "editable"
    editable.mkdir(parents=True)
    (editable / "timeline.json").write_text('{"version":"1.0"}', encoding="utf-8")
    media = editable / "media"
    media.mkdir()
    (media / "shot.mp4").write_bytes(b"proxy")
    ...
    result = ExportBundle().execute(
        {
            ...
            "title": "Editable Demo",
            "export_dir": str(tmp_path / "out"),
            "editable_project_dir": str(editable),
        }
    )

    assert result.success is True
    copied = Path(result.data["editable_project_path"])
    assert (copied / "timeline.json").is_file()
    assert (copied / "media" / "shot.mp4").read_bytes() == b"proxy"
